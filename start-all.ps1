...
param(
    ...
    [int]   $TimeoutSecs   = 180,
    [switch]$StartFrontend        # pass -StartFrontend to also launch the Angular dev server
)
...
# ----------------------------------------------------------------------------
#  LAUNCH ANGULAR DEV SERVER IN ITS OWN WINDOW
# ----------------------------------------------------------------------------
function Start-Frontend {
    ...
    $batLines = @(
        "@echo off",
        "title Angular Frontend  [port 4200]",
        "color 0D",
        "echo ============================================================",
    ...
    $batPath = Join-Path ([System.IO.Path]::GetTempPath()) "medlab_frontend.bat"
...
$services = @(
    ...
    @{
        ...
        Dir  = Join-Path $BACKEND "inventory-service" | Join-Path -ChildPath "inventory-service"
        Port = 8084
        Env  = @{}
    },
    @{
    ...
    @{
        ...
        Dir  = Join-Path $BACKEND "lab-processing-service" | Join-Path -ChildPath "lps"
        Port = 8083
        Env  = @{}
    },
    @{
...
if ($failed.Count -gt 0) {
    ...
    Write-OK "All 10 backend services are running."
}

# -- Step 8 (optional): Start Angular frontend --------------------------------
if ($StartFrontend) {
    Write-Host ""
    Write-Step "Starting Angular frontend..."
    $frontendDir = Join-Path $ROOT "frontend"
    if (-not (Test-Path $frontendDir)) {
        Write-Err "Frontend directory not found: $frontendDir"
    } elseif (-not (Find-NodeExe)) {
        Write-Err "Node.js not found in PATH or common locations."
        Write-Err "Install Node.js from https://nodejs.org and retry, or run frontend-dev.bat separately."
    } else {
        $nodeModules = Join-Path $frontendDir "node_modules"
        if (-not (Test-Path $nodeModules)) {
            Write-Warn "node_modules not found -- running 'npm install' first. This may take a minute..."
            $npmCmd = Get-Command "npm" -ErrorAction SilentlyContinue
            $npmExe = if ($npmCmd) { $npmCmd.Source } else { "npm" }
            & $npmExe install --prefix "$frontendDir" | Out-Null
        }
        Start-Frontend -FrontendDir $frontendDir
        Write-Host ""
        Write-Host "    Waiting for Angular dev server on port 4200 " -NoNewline
        $deadline = (Get-Date).AddSeconds(120)
        $up = $false
        while ((Get-Date) -lt $deadline) {
            try {
                $resp = Invoke-WebRequest -Uri "http://localhost:4200" -TimeoutSec 2 -UseBasicParsing -ErrorAction Stop
                $up = $true; break
            } catch {}
            Write-Host "." -NoNewline
        ...
        if ($up) {
            Write-Host " UP" -ForegroundColor Green
            Write-OK "Angular frontend is UP"
            Write-Host "  [UP    ]  Angular Frontend                           port 4200" -ForegroundColor Green
        } else {
            Write-Host " TIMED OUT" -ForegroundColor Red
            Write-Warn "Angular dev server did not respond within 120 s. Check the frontend window."
        }
    }
}
...
if ($failed.Count -eq 0) {
    ...
    if ($StartFrontend) {
        Write-Host "  -> Open http://localhost:4200 in your browser" -ForegroundColor Gray
    } else {
        ...
        Write-Host "  4. Start frontend  -> run frontend-dev.bat  (or re-run start-all.bat and answer Y)" -ForegroundColor Gray
    }
    Write-Host ""
}
