...
.DESCRIPTION
    ...
    Uses the api gateway port 8090 ONLY, NOT the individual service ports (8081-8087).
.EXAMPLE
    .\test-all.ps1
...
$LPS    = "http://localhost:8090"
$INV    = "http://localhost:8090"
$BILL   = "http://localhost:8090"
$PAT    = "http://localhost:8090"
$NOTIFY = "http://localhost:8090"
...
function Write-Banner {
    ...
    Write-Host "|   API Gateway port (8090) ONLY, NOT individual ports        |" -ForegroundColor Cyan
    Write-Host "+============================================================+" -ForegroundColor Cyan
    Write-Host ""
}
...
$svcMap = [ordered]@{
    ...
    "order-service    :8090" = 8090
    "lps              :8090" = 8090
    "inventory-service:8090" = 8090
    "billing-service  :8090" = 8090
    "patient-service  :8090" = 8090
    "notification-svc :8090" = 8090
}
$anyDown = $false
...
if ($rProf.Code -eq 200) {
    $profId = if ($rProf.Json.id)            { [long]$rProf.Json.id }
              elseif ($rProf.Json.patientId)  { [long]$rProf.Json.patientId }
              elseif ($rProf.Json.profileId)  { [long]$rProf.Json.profileId }
              else { 0 }
    if ($profId -gt 0) {
        $patientProfileId = $profId
        Write-Host "         patientProfileId=$patientProfileId (patient_db.patient PK, from GET /patient/profile)" -ForegroundColor DarkGray
    } else {
        Write-Host "         WARNING: could not resolve patientProfileId from GET /patient/profile (HTTP 200)" -ForegroundColor Yellow
    }
} else {
...
if ($mysqlFound -and $patientUserId -gt 0) {
    $profCnt = $null; $foundPatTable = ""
    foreach ($tbl in @("patient", "patients", "patient_profile", "patient_profiles", "patient_info")) {
        $cnt = SQL-Scalar "SELECT COUNT(*) FROM patient_db.${tbl} WHERE user_id=$patientUserId;"
        if ($null -ne $cnt) { $profCnt = $cnt; $foundPatTable = $tbl; break }
    }
    if ($null -ne $profCnt) {
        if ([int]$profCnt -ge 1) { PASS "8.1-DB Patient profile row exists in patient_db.$foundPatTable" "user_id=$patientUserId" }
        else { FAIL "8.1-DB Patient profile row missing" "patient_db.$foundPatTable has no row for user_id=$patientUserId" }
...
if ($patientProfileId -eq 0 -and $dbInvPatientId -and [long]$dbInvPatientId -gt 0) {
    $patientProfileId = [long]$dbInvPatientId
    Write-Host "         patientProfileId=$patientProfileId (resolved from billing.invoices.patient_id)" -ForegroundColor DarkGray
}
...
if (-not $gwUp) {
    ...
    Test-Via-Gateway "GW-1 Auth: POST /auth/login (gateway:8090 vs :8090)" `
    ...
    if ($adminToken) {
        Test-Via-Gateway "GW-2 Auth: GET /admin/users (gateway:8090 vs :8090)" `
    ...
    if ($adminToken) {
        Test-Via-Gateway "GW-3 Inventory: GET /tests (gateway:8090 vs :8090)" `
    ...
    if ($adminToken) {
        Test-Via-Gateway "GW-4 Order: GET /orders/viewAllOrders (gateway:8090 vs :8090)" `
    ...
    if ($labTechToken) {
        Test-Via-Gateway "GW-5 LPS: GET /api/jobs (gateway:8090 vs :8090)" `
    ...
    if ($patientToken) {
        ...
        Test-Via-Gateway "GW-6 Billing: GET $gwBillPath (gateway:8090 vs :8090)" `
    ...
    # Gateway routes /patient/profile → user-service (auth, :8090) per the route
    ...
    if ($patientToken) {
        Test-Via-Gateway "GW-7 Auth: GET /patient/profile (gateway:8090 -> :8090)" `
    ...
    if ($patientToken) {
        ...
        Test-Via-Gateway "GW-7b Patient: GET /patient/by-id/$gwPatId (gateway:8090 vs :8090)" `
    ...
    if ($patientToken) {
        Test-Via-Gateway "GW-8 Notification: GET /notification (gateway:8090 vs :8090)" `
...
# ═══════════════════════════════════════════════════════════════════════════
#  S.FE  Angular Frontend Tests
#  Verifies the Angular dev server (ng serve on :4200) responds correctly.
#  Every test here is automatically SKIPPED when port 4200 is not open —
#  start the frontend first with:  start-all.bat  (answer Y)
#                               or frontend-dev.bat serve
# ═══════════════════════════════════════════════════════════════════════════
Write-Section "S.FE -- Angular Frontend (ng serve, port 4200)"

# ── FE-0  Port check (with up-to-90s retry so Angular has time to compile) ───
# NOTE: Node.js v17+ resolves "localhost" to ::1 (IPv6) on modern Windows.
#       A raw TcpClient("127.0.0.1") would always miss that. Use an HTTP GET
#       against http://localhost:4200 instead -- .NET resolves the hostname
#       correctly regardless of whether Angular bound to IPv4 or IPv6.
$feUp = $false
$feWaitMax = 90   # seconds to wait for Angular to come up
$feWaitSw  = [System.Diagnostics.Stopwatch]::StartNew()
Write-Host "    Waiting up to $feWaitMax s for Angular dev server on port 4200..." -ForegroundColor DarkGray
while (-not $feUp -and $feWaitSw.Elapsed.TotalSeconds -lt $feWaitMax) {
    try {
        $r = Invoke-WebRequest -Uri "http://localhost:4200" -UseBasicParsing -TimeoutSec 2 -ErrorAction Stop
        if ($r.StatusCode -eq 200) { $feUp = $true; break }
    } catch {}
    if (-not $feUp) { Start-Sleep -Seconds 3 }
}
$feWaitSw.Stop()
if ($feUp) {
    Write-Host "    Angular up after $([int]$feWaitSw.Elapsed.TotalSeconds) s." -ForegroundColor DarkGray
}

if (-not $feUp) {
    Write-Host ""
    Write-Host "    Angular dev server (port 4200) is NOT running (waited $feWaitMax s)." -ForegroundColor Yellow
    Write-Host "    Start it: run debug.bat (answer Y) or frontend-dev.bat serve" -ForegroundColor Yellow
    foreach ($name in @(
        "FE-0  Dev server up (port 4200)",
        "FE-1  Root GET / returns 200 with Angular HTML",
        "FE-2  <app-root> present in index.html",
        "FE-3  Angular Material icons link present",
        "FE-3b main.js chunk served (200)",
        "FE-3c polyfills.js chunk served (200)",
        "FE-3d runtime.js chunk served (200)",
        "FE-3e styles.css served (200)",
        "FE-4  SPA routing: /auth/login returns 200",
        "FE-4b SPA routing: /dashboard returns 200",
        "FE-4c SPA routing: /billing returns 200",
        "FE-4d SPA routing: unknown route returns 200 (SPA fallback)",
        "FE-5  API gateway CORS preflight from frontend origin",
        "FE-6  Login API reachable via gateway (admin credentials)",
        "FE-7  Angular bundle sanity (main.js > 100 KB)"
    )) { SKIP $name "Angular dev server not running on port 4200" }
} else {
    PASS "FE-0  Dev server up (port 4200)"

    # Helper: plain HTTP GET (no auth, expects HTML/JS/CSS)
    function FE-Get {
        param([string]$Url)
        try {
            ...
            return @{ Code=$code; Body=""; OK=$false }
        }
    }

    # ── FE-1  Root returns 200 ────────────────────────────────────────────────
    $feRoot = FE-Get "http://localhost:4200/"
    if ($feRoot.Code -eq 200) { PASS "FE-1  Root GET / returns 200 with Angular HTML" }
    else { FAIL "FE-1  Root GET /" "HTTP $($feRoot.Code)" }

    # ── FE-2  index.html contains <app-root ──────────────────────────────────
    if ($feRoot.OK) {
        if ($feRoot.Body -match '<app-root') {
            PASS "FE-2  <app-root> present in index.html"
        } else {
            FAIL "FE-2  <app-root> missing from index.html" "HTML may not be the Angular shell"
        }
    } else { SKIP "FE-2  <app-root> check" "FE-1 failed" }

    # ── FE-3  Angular Material icons link ────────────────────────────────────
    if ($feRoot.OK) {
        # Note: 'Material+Icons' uses \+ to match the literal plus in the Google Fonts URL.
        # Without the backslash, + is a regex quantifier (one-or-more), not a literal plus.
        $hasMaterialIcons = $feRoot.Body -match 'material-icons' -or $feRoot.Body -match 'Material\+Icons' -or $feRoot.Body -match 'fonts\.googleapis\.com.*icon'
        $hasMatTheme      = $feRoot.Body -match 'mat-' -or $feRoot.Body -match 'angular'
        if ($hasMaterialIcons) {
            PASS "FE-3  Angular Material icons link present in index.html"
        } elseif ($hasMatTheme) {
            PASS "FE-3  Angular Material reference present in index.html (mat- classes)"
        } else {
            FAIL "FE-3  Angular Material not detected in index.html" "Check src/index.html for Material icons link"
        }
    } else { SKIP "FE-3  Material icons check" "FE-1 failed" }

    # ── FE-3b/c/d/e  Core static chunks ──────────────────────────────────────
    foreach ($chunk in @(
        @{ Name="FE-3b main.js chunk";     Path="main.js"      },
        @{ Name="FE-3c polyfills.js chunk"; Path="polyfills.js" },
        @{ Name="FE-3d runtime.js chunk";   Path="runtime.js"   },
        @{ Name="FE-3e styles.css";         Path="styles.css"   }
    )) {
        $cr = FE-Get "http://localhost:4200/$($chunk.Path)"
        if ($cr.Code -eq 200) { PASS "$($chunk.Name) served (200)" "size=$($cr.Body.Length) bytes" }
        else { FAIL "$($chunk.Name) not served" "HTTP $($cr.Code) for /$($chunk.Path)" }
    }

    # ── FE-4  SPA routing — Angular routes must all return index.html (200) ──
    # webpack-dev-server (used by ng serve) only applies historyApiFallback for
    # requests that include "text/html" in the Accept header.  Without it,
    # non-browser HTTP clients get a real 404.  Add the header here to replicate
    # what a browser would send when navigating to a deep-link URL.
    foreach ($route in @(
        @{ Name="FE-4  SPA routing: /auth/login";           Path="/auth/login"       },
        @{ Name="FE-4b SPA routing: /dashboard";            Path="/dashboard"        },
        @{ Name="FE-4c SPA routing: /billing";              Path="/billing"          },
        @{ Name="FE-4d SPA routing: unknown route /x/y/z";  Path="/x/y/z"           }
    )) {
        $sr = $null
        try {
            $spr = Invoke-WebRequest -Uri "http://localhost:4200$($route.Path)" `
                       -UseBasicParsing -TimeoutSec 10 `
                       -Headers @{ "Accept" = "text/html,application/xhtml+xml,*/*" } `
                       -ErrorAction Stop
            $sr = @{ Code = [int]$spr.StatusCode; Body = $spr.Content; OK = $true }
        } catch {
            $xc = $_.Exception
            if ($xc.Response) { $sr = @{ Code = [int]$xc.Response.StatusCode; Body = ""; OK = $false } }
            else               { $sr = @{ Code = 0; Body = ""; OK = $false } }
        }
        if ($sr.Code -eq 200 -and $sr.Body -match '<app-root') {
            PASS "$($route.Name) returns 200 with Angular shell"
        } elseif ($sr.Code -eq 200) {
            PASS "$($route.Name) returns 200 (SPA)"
        } else {
            FAIL "$($route.Name) did not return 200" "HTTP $($sr.Code) - SPA fallback may not be configured"
        }
    }

    # ── FE-5  CORS preflight from frontend origin ────────────────────────────
    Write-Host ""
    Write-Host "  [FE-5] API Gateway CORS preflight (Origin: http://localhost:4200)" -ForegroundColor White
    try {
        $corsHeaders = @{
        ...
        $corsr = Invoke-WebRequest -Uri "http://localhost:8090/auth/login" `
                     -Method OPTIONS -Headers $corsHeaders `
                     -UseBasicParsing -TimeoutSec 8 -ErrorAction Stop
        $code     = [int]$corsr.StatusCode
        $acaoHdr  = $corsr.Headers["Access-Control-Allow-Origin"]
        $acamHdr  = $corsr.Headers["Access-Control-Allow-Methods"]

        if ($code -in 200,204) {
            if ($acaoHdr -match "localhost:4200" -or $acaoHdr -eq "*") {
                PASS "FE-5  CORS preflight accepted, Access-Control-Allow-Origin includes frontend" "ACAO=$acaoHdr"
            } else {
                PASS "FE-5  CORS preflight returned $code (ACAO=$acaoHdr - may still work for requests without credentials)"
            }
        } else {
            FAIL "FE-5  CORS preflight unexpected status" "HTTP $code"
        }
    } catch {
        $errCode = 0
        if ($_.Exception.Response) { $errCode = [int]$_.Exception.Response.StatusCode }
        # 403/405 on OPTIONS means gateway blocked the preflight — CORS will fail in browser
        if ($errCode -in 403,405) {
            FAIL "FE-5  CORS preflight blocked by gateway" "HTTP $errCode - browser requests from :4200 will fail with CORS error"
        } else {
            SKIP "FE-5  CORS preflight" "Gateway not responding or OPTIONS not supported (HTTP $errCode)"
        }
    }

    # ── FE-6  Login API reachable via gateway from frontend's perspective ─────
    Write-Host ""
    Write-Host "  [FE-6] Login API reachable via gateway (POST /auth/login)" -ForegroundColor White
    $fe6Headers = @{
        "Content-Type" = "application/json"
        "Origin"       = "http://localhost:4200"
    }
    try {
        $fe6r = Invoke-WebRequest -Uri "http://localhost:8090/auth/login" `
                    -Method POST `
                    -Headers $fe6Headers `
                    -Body '{"username":"admin@lab.com","password":"Admin123"}' `
                    -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop
        $fe6Code = [int]$fe6r.StatusCode
        try {
            $fe6Json = $fe6r.Content | ConvertFrom-Json
            $hasToken = $fe6Json.token -or $fe6Json.accessToken -or $fe6Json.jwt
        } catch { $hasToken = $false }

        if ($fe6Code -eq 200 -and $hasToken) {
            PASS "FE-6  Login API reachable and returns JWT token" "HTTP 200"
        } elseif ($fe6Code -eq 200) {
            PASS "FE-6  Login API reachable (200) - check token field name if frontend login fails" "response: $($fe6r.Content.Substring(0, [Math]::Min(80,$fe6r.Content.Length)))"
        } else {
            FAIL "FE-6  Login API unexpected response" "HTTP $fe6Code"
        }
    } catch {
        $fe6Err = 0
        if ($_.Exception.Response) { $fe6Err = [int]$_.Exception.Response.StatusCode }
        if ($fe6Err -eq 401) {
            FAIL "FE-6  Login API returned 401 for admin credentials" "Check admin@lab.com exists in auth_db"
        } elseif ($fe6Err -gt 0) {
            FAIL "FE-6  Login API error" "HTTP $fe6Err"
        } else {
            FAIL "FE-6  Login API unreachable" "$($_.Exception.Message)"
        }
    }

    # ── FE-7  Bundle sanity: main.js should be a real Angular bundle (> 100 KB) ─
    Write-Host ""
    Write-Host "  [FE-7] Angular bundle sanity check (main.js size)" -ForegroundColor White
    $fe7r = FE-Get "http://localhost:4200/main.js"
    if ($fe7r.Code -eq 200) {
        $sizeKb = [int]($fe7r.Body.Length / 1024)
        if ($sizeKb -gt 100) {
            PASS "FE-7  Angular bundle sanity: main.js is ${sizeKb} KB (genuine build)" "size=${sizeKb}KB > 100KB"
        } else {
            FAIL "FE-7  main.js is unexpectedly small (${sizeKb} KB)" "Expected > 100 KB for a real Angular Material app"
        }
    } else { SKIP "FE-7  Bundle size check" "main.js not served (HTTP $($fe7r.Code))" }
}

# ═══════════════════════════════════════════════════════════════════════════
#  S.E2E  Playwright Browser Tests
#  Runs only when port 4200 is up AND the e2e/ folder + node_modules exist.
#  Each individual Playwright result is recorded as PASS/FAIL/SKIP here.
# ═══════════════════════════════════════════════════════════════════════════
Write-Section "S.E2E -- Playwright Browser E2E Tests"

$e2eDir    = Join-Path $PSScriptRoot "testing\e2e"
$e2eNmDir  = Join-Path $e2eDir "node_modules"
$e2eScript = Join-Path $e2eDir "tests\medlab-e2e.spec.js"

# Reuse the $feUp result from S.FE (which already waited up to 90 s for port 4200)
$port4200Up = $feUp

if (-not $port4200Up) {
    Write-Host "    Angular dev server (port 4200) is NOT running -- skipping E2E tests." -ForegroundColor Yellow
    Write-Host "    Start it with debug.bat (answer Y) or frontend-dev.bat serve" -ForegroundColor Yellow
    $e2eNames = @(
        "PUB-1 Login page renders","PUB-2 Register page renders","PUB-3 Unknown route 404",
        "PUB-4 /dashboard redirects unauthenticated","PUB-5 /admin redirects unauthenticated",
        "AUTH-1 Wrong password error","AUTH-2 Empty form validation","AUTH-3 Admin login",
        "AUTH-4 Lab Tech login","AUTH-5 Patient login","AUTH-6 Logout clears token",
        "ADMIN-1 Dashboard no JS errors","ADMIN-2 Sidenav visible","ADMIN-3 /admin user list",
        "ADMIN-4 create-lab-tech form","ADMIN-5 create-lab-tech validation",
        "ADMIN-6 /inventory page","ADMIN-7 /orders page","ADMIN-8 /billing page",
        "ADMIN-9 /lab page","ADMIN-10 PATIENT pages block ADMIN",
        "LAB-1 Dashboard lab tech","LAB-2 /lab page","LAB-3 /admin blocked for lab tech",
        "PAT-1 Dashboard patient","PAT-2 /notifications","PAT-3 /orders patient","PAT-4 /admin blocked for patient",
        "VIS-1 Material form dimensions","VIS-2 No horizontal overflow login",
        "VIS-3 No overflow dashboard","VIS-4 Mobile 375px layout",
        "VIS-5 No uncaught errors all pages","VIS-6 Page title not blank",
        "VIS-7 Toolbar renders","VIS-8 No broken images","VIS-9 API calls reach gateway",
        "VIS-10 Register password mismatch"
    )
    foreach ($n in $e2eNames) { SKIP "E2E $n" "Angular not running on port 4200" }
} elseif (-not (Test-Path $e2eNmDir)) {
    Write-Host "    e2e/node_modules not found -- running npm install + playwright install..." -ForegroundColor Yellow
    Push-Location $e2eDir
    & npm install 2>&1 | Out-Null
    & npx playwright install chromium 2>&1 | Out-Null
    Pop-Location
    Write-Host "    Playwright installed. Re-run debug.bat to execute E2E tests." -ForegroundColor Yellow
    SKIP "E2E browser tests (all)" "First-run install completed -- re-run to execute tests"
} else {
    Write-Host "    Running Playwright (headless Chromium)..." -ForegroundColor Gray
    Write-Host ""

    # Run playwright and capture JSON reporter output
    $jsonOut = Join-Path $env:TEMP "pw_results.json"
    Push-Location $e2eDir
    $pwProc = Start-Process -FilePath "npx" `
        -ArgumentList "playwright", "test", "--reporter=json" `
        -NoNewWindow -PassThru -Wait `
        -RedirectStandardOutput $jsonOut `
        -RedirectStandardError  "$env:TEMP\pw_stderr.txt"
    Pop-Location

    # Parse JSON output for individual test results
    $e2ePass = 0; $e2eFail = 0; $e2eSkip = 0
    try {
        $pwJson = Get-Content $jsonOut -Raw -ErrorAction Stop | ConvertFrom-Json
        foreach ($suite in $pwJson.suites) {
            foreach ($innerSuite in $suite.suites) {
                foreach ($spec in $innerSuite.specs) {
                    $title = "$($innerSuite.title) - $($spec.title)"
                    $result = $spec.tests[0].results[0].status
                    switch ($result) {
                        "passed"  { PASS "E2E $title"; $e2ePass++ }
                        "failed"  {
                            $errMsg = $spec.tests[0].results[0].error.message
                            $short  = if ($errMsg -and $errMsg.Length -gt 120) { $errMsg.Substring(0,120) + "..." } else { $errMsg }
                            FAIL "E2E $title" $short
                            $e2eFail++
                        }
                        "skipped" { SKIP "E2E $title" "skipped"; $e2eSkip++ }
                        default   { SKIP "E2E $title" "unknown status: $result"; $e2eSkip++ }
                    }
                }
            }
        }
        Write-Host ""
        Write-Host "  E2E Browser: PASS=$e2ePass  FAIL=$e2eFail  SKIP=$e2eSkip" -ForegroundColor Cyan
        if ($e2eFail -gt 0) {
            Write-Host "  HTML report: $PSScriptRoot\playwright-report\index.html" -ForegroundColor Yellow
            Write-Host "  Screenshots: $PSScriptRoot\playwright-results\" -ForegroundColor Yellow
        }
    } catch {
        Write-Host "    Could not parse Playwright JSON output: $_" -ForegroundColor Yellow
        Write-Host "    Run 'cd e2e && npx playwright test' manually to see results." -ForegroundColor Yellow
        SKIP "E2E browser tests (all)" "Playwright output parse error"
    }
}
...
Write-Host ""
Write-Host "+============================================================+" -ForegroundColor Cyan
Write-Host "|   TEST SUMMARY  (backend API + Angular frontend)           |" -ForegroundColor Cyan
Write-Host "+============================================================+" -ForegroundColor Cyan
Write-Host ""
