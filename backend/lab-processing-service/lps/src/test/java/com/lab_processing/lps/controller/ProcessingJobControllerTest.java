...
class ProcessingJobControllerTest {
    ...
    void approveResult_delegatesToService() {
        doNothing().when(processingJobService).approveResult(1L, 2L);
        processingJobController.approveResult(1L, 2L);
        verify(processingJobService, times(1)).approveResult(1L, 2L);
    }
}